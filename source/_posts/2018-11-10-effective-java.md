---
layout: post
title: "Effective Java 3th 정리노트"
description: "책 읽고 간단히 정리한 흔적들"
date: 2018-11-10
tags: [java, effective]
comments: true
share: true
image: '/asset/images/effective/effective.webp'
thumbnail: '/asset/images/effective/effective.webp'
categories: ['Effective Java']
---

## Effective Java 3th 정리노트

- [Item 1 객체 생성 정적 팩토리 메서드를 쓰는게 왜 유리한가](/2018/11/12/static-creator)
- [Item 2 생성자에 매개변수가 많다면 빌더 패턴을 고려하라](/2018/11/14/builder)
- [Item 3 private 생성자나 열거 타입으로 싱글턴임을 보장하라](/2018/11/19/serialize-java)
- [Item 4 인스턴스화를 막으려거든 private 생성자를 사용하라](/2018/11/20/no-instance)
- [Item 5 자원을 직접 명시하지 말고 의존 객체 주입을 사용하라](/2018/11/20/dependency-inject)
- [Item 6 불필요한 객체 생성을 피하라](/2018/11/25/불필요한-객체생성-회피)
- [Item 8,9 finalizer 와 cleaner 사용을 피하라 & Try-With-Resource](/2018/11/26/Finalizer와-Cleaner)
- [Item 10 equals 는 일반 규약을 지켜 재정의하라](/2018/11/27/object-equals-liskov)
- ...