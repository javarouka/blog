...
제네릭을 처음 공부할때 위에서 설명한 수퍼/서브타입 덕분에 많이 헷갈려하는 부분이 제네릭 파라미터가 수퍼/서브타입의 관계일 경우 제네릭 클래스도 수퍼/서브타입이 성립할거라는 착각이다.
...
- 생성자에서 사용되는 `super` 는 *생성자 함수* 를 가르킨다. 생성자에서의 `super` 는 바로 호출하는 함수 형태이다.
- 멤버 함수에서의 `super` 는 *인스턴스* 를 가르킨다. 메서드에서의 `super` 는 부모 인스턴스를 레퍼런스하는 변수처럼 동작한다. 
