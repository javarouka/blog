...
보통 프로그래밍 언어에서는 함수가 호출될 경우 함수들은 자신을 호출한 곳으로 되돌아갈 곳을 알아야 한다. 이 정보는 Call Stack 이라는 함수의 호출 순서대로 입력되는 Stack 으로 관리된다.
...
다음 포스트에서 알아볼 Promise.all 과 같이 쓰면 코드가 더욱 간결해질 것이다.
