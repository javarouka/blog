...
setTimeout, setInterval 로 몇초뒤의 작업을 예약하면 `Job Queue` 에 추가되는 것이 아니라 Timer Api 에서 해당 시간만큼 지연된 뒤에 `Job Queue` 에 추가한다.
...
이상을 종합해보면 javascript 는 단지 `Call Stack` 을 처리해갈 뿐이지만 어떤 상황으로 인해 별도로 준비된 Job Queue 에 작업이 추가되고, Event Loop 에서 그것을 꺼내 Call Stack 에 추가하면 다시 실행하고... 이것을 반복하여 처리된다.
